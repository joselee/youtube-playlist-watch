...
class Downloader extends EventEmitter {
    ...
    fetchAndEncode(videoInfo, callback) {
        ...
        ytdl.getInfo(videoUrl, { quality: this.youtubeVideoQuality }, (err, info) => {
            if (err) {
                callback(err.message, videoInfo); return;
            }

            const videoTitle = this.cleanFileName(info.title);
            const fileName = `${videoInfo.index} ${videoTitle}.mp3`;
            const filePath = `${this.outputPath}/${videoInfo.index} ${videoTitle}.mp3`;
            ...
            stream.on("response", (httpResponse) => {
                ...
                encoding.on("end", () => callback(null, videoInfo));
                encoding.saveToFile(filePath);
            });
        });
    }
}
